...
- `src/z3rno_core/retrieval/redaction.py` — **Phase F slice 2:** `RetrievalFilter` ABC + `RedactionFilter` impl; role-aware regex scrubbing applied to results/metadata/graph_context after rerank. Built-in defaults (email/SSN/card/phone) + optional YAML rules file with per-role overrides and `fallback_role`. Opt-in via `engine.recall(role=..., retrieval_filters=[...])`.
- `src/z3rno_core/forget_proof/` — **Phase F slice 5 — forget-with-proof:** Merkle tree builder over `(memory_id, content_hash, audit_hash)` leaves + ed25519 sign/verify + PEM key loader. `engine.forget(signing_key=..., signer_key_id=...)` emits a row in `forget_certificates` with the signed root; the verifier CLI lives in z3rno-evals and is z3rno-core-free.
- `src/z3rno_core/conversations/` — **Phase G slice 2 — conversation memory:** `Conversation` + `Turn` primitives, atomic `add_turn` (bumps `conversation.turn_count`, stamps `memories.conversation_id/turn_index/turn_role`), `needs_summary`/`mark_summary_emitted` for cadence tracking. `engine.recall(conversation_id=...)` scopes retrieval to one session via `_filters.build_where_clause`.
